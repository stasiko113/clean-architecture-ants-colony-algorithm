...
export class NodeMapperProfile implements MapperProfileInterface<NodeModel, NodeEntity> {
	...
	toModel(entityOrEntities: NodeEntity | NodeEntity[]): NodeModel | NodeModel[] {
		if (Array.isArray(entityOrEntities)) {
			return entityOrEntities.map(entity => this.mapSingleEntityToModel(entity));
		} else {
